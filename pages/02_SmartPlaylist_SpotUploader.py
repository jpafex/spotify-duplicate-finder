            ...
            if spotify_status and spotify_status["status"] == "unauthorized":
                ...
                if st.button("📤 Create Spotify Playlist", type="secondary"):
                    with st.spinner("Communicating with Spotify..."):
                        try:
                            # FIX 1: Bypass spotipy's broken wrapper and use the new 2026 /me/playlists endpoint
                            playlist = sp._post(
                                "me/playlists", 
                                payload={
                                    "name": spotify_playlist_name,
                                    "public": False,
                                    "description": "Generated by Afex ProDJ Harmonic Flow Engine"
                                }
                            )
                           ...
                           # Extract track URIs (Safely finding the column regardless of capitalization)
                            ...
                            if not track_uris:
                                ...
                                for i in range(0, len(track_uris), 100):
                                    ...
                                    # FIX 2: Use the new 2026 /items endpoint instead of /tracks
                                    sp._post(f"playlists/{playlist_id}/items", payload={"uris": batch_uris})
