...
st.title("📦 Dropbox Bridge (Precision Parser)")

# 2. THE REFINED PARSER
def parse_music_filename(filename):
    """
    KAIZEN: Smart Separator Logic.
    Ignores hyphens inside names (like K-Paz) by requiring spaces around the separator.
    """
    ...
    # SMART PATTERN: Requires spaces around the dash/underscore to protect artist names
    ...
    # Fallback for filenames without ' - ' (Treats whole name as Title)
...
if 'cloud_inventory' in st.session_state:
    ...
    st.download_button(
        ...
        data=df[['Name', 'Artist', 'Album', 'Folder', 'Full Path']].to_csv(index=False).encode('utf-8-sig'),
        file_name=f"Dropbox_Precision_Inventory_{datetime.now().strftime('%Y%m%d')}.csv",
        mime="text/csv",
