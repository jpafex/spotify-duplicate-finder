...
# Import the same bootstrap used in your other pages
from afexcloud.layout import bootstrap_page

# Page Configuration
st.set_page_config(
    page_title="MP3 Metadata Tag Writer", 
    page_icon="🏷️", 
    layout="wide"
)
...
# App Header
st.title("🏷️ ID3 Tag Writer")
st.markdown("Batch write Camelot Keys, BPM, and standard metadata directly to your local MP3 files.")
...
def convert_spotify_key_to_camelot(spotify_key, mode):
    ...
    if mode == 1:
        return major_map.get(spotify_key, 'Unknown')
    else:
        return minor_map.get(spotify_key, 'Unknown')

def find_mp3_file(track_name, artist_name, duration_ms, folder):
    folder = Path(folder)
...
def write_metadata(mp3_path, row, dry_run=True):
    if dry_run:
...
with st.expander("⚙️ Configuration", expanded=True):
    mp3_directory = st.text_input("Local MP3 Folder Path", value=r"D:\Afex_Music_Python\mp3s")
    dry_run_mode = st.toggle("🧪 DRY RUN mode (Test without modifying files)", value=True)

uploaded_file = st.file_uploader("Upload Target Data (.csv)", type=["csv"])
...
if uploaded_file is not None:
    ...
    if st.button("🚀 Execute Tagging Process", type="primary"):
        if not os.path.exists(mp3_directory):
            st.error(f"❌ Cannot find MP3 directory: {mp3_directory}")
        else:
            ...
            for index, row in df.iterrows():
                ...
                status_text.text(f"Processing ({index + 1}/{total_tracks}): {track} - {artist}")
                
                mp3_path = find_mp3_file(track, artist, duration, mp3_directory)
                
                if mp3_path:
                    success, msg = write_metadata(mp3_path, row, dry_run=dry_run_mode)
                    if success:
                        success_count += 1
                    else:
                        missing_tracks.append(f"{track} - {artist} (Error: {msg})")
                else:
                    missing_tracks.append(f"{track} - {artist} (Not Found)")
            ...
            # --- SUMMARY ---
            st.success(f"📊 Summary: Successfully processed {success_count} out of {total_tracks} tracks.")
