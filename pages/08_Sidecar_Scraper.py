...
SIDECAR_CODE_VERSION = "1.0.2"  # Updated for cache busting
...
def get_access_token_from_session() -> str | None:
    """Get Spotify access token from session state"""
...
def spotify_call(func, *args, max_retries: int = 5, base_sleep: float = 0.8, **kwargs):
    """Wrapper for Spotify API calls with retry logic"""
    attempt = 0
    while True:
        try:
            ...
            # For 404 errors, don't retry - they won't succeed
            if status == 404:
                raise
...
def fetch_audio_features_safe(sp: spotipy.Spotify, track_ids: list[str]) -> dict[str, dict | None]:
    ...
    def helper(chunk: list[str]):
        ...
        try:
            ...
            status = getattr(e, "http_status", None)
            if status in (400, 403, 404) and len(chunk) > 1:
                mid = len(chunk) // 2
                helper(chunk[:mid])
                helper(chunk[mid:])
                return
...
def analyze_playlist_cached(
    ...
    # FIX: Remove additional_types parameter for playlist endpoint
    # The playlist endpoint doesn't support additional_types parameter
    try:
        # Try without additional_types first
        pl = spotify_call(
            sp.playlist, 
            playlist_id, 
        ...
        if getattr(e, "http_status", None) == 404:
            # Try a different approach - get minimal info
            try:
    ...
    while offset < target_total:
        try:
            batch = spotify_call(
                ...
                additional_types="track"  # This is allowed for playlist_items
            )
        except SpotifyException as e:
            if getattr(e, "http_status", None) == 404:
                st.error("Playlist not found or inaccessible. Make sure:")
                st.error("1. The playlist ID is correct")
                st.error("2. The playlist is public or shared with your account")
                break
            raise
...
def run_analysis(playlist_input: str, max_tracks: int):
    ...
    with st.spinner("Analyzing playlist..."):
        try:
            ...
            if status == 401:
                ...
                st.error("Playlist not found. Please check the playlist URL/ID and make sure it's accessible.")
                return
            st.error(f"Spotify returned an error: HTTP {status}")
            return
    ...
    if df is None or df.empty:
        if diag.get("kept_tracks", 0) == 0:
            st.warning("No tracks found. Possible reasons:")
            st.markdown("""
            - Playlist is empty
            ...
            st.warning("No audio features retrieved for tracks.")
        return
...
if option.startswith("1"):
    ...
    max_tracks = st.number_input("Max tracks (0 = ALL)", min_value=0, value=50, step=10)

    if st.button("🚀 Analyze Playlist"):
        run_analysis(playlist_input, int(max_tracks))

elif option.startswith("2"):
    # Try a different test playlist - the "Today's Top Hits" playlist
    test_url = "https://open.spotify.com/playlist/37i9dQZF1DXcBWIGoYBM5M"
    st.write("Example playlist URL (Today's Top Hits):")
    ...
    st.markdown(
        ...
        **Troubleshooting common issues:**
        1. **404 errors**: Make sure playlist is public or shared with your account
        2. **No results**: Try clearing cache and re-running
        3. **Authentication errors**: Reconnect Spotify in sidebar

        **Version:** 1.0.2 (Fixed 404 error issue)
        """
    )
